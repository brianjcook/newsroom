...
- archive cards now use the computed rank internally but no longer show the rank publicly
...
- Removed public exposure of editorial scoring/ranking internals:
- event pages no longer show numeric scores, coverage modes, or factor weights in the sidebar
- event sidebars now use `You might also like` related links or reader-facing calendar/topic fallbacks
- `/calendar` no longer prints community-event scores
- `/archive` no longer prints internal archive ranks
- homepage copy now uses reader-facing labels like `Local Priorities`, `Featured Listing`, and `Latest source refresh` instead of `Desk Priorities`, `Event Tiers`, and worker-run wording
- public byline subtitles now avoid `Automated civic reporting` and `listings desk` labels
- Added a public-copy quality pass in `worker/newsroom/publish.py` with render version `2026-05-12-render-v20-public-copy-lede`:
- preview deks now combine meeting time with the main agenda issues instead of only restating the meeting schedule
- preview ledes now identify the leading agenda item first, then give the meeting time/location, while avoiding singular/plural agreement problems
- minutes recap headlines now use `Minutes Highlight {issue}` when focus items exist instead of defaulting to `Wareham {body} minutes posted`
- public dates generated by the worker now avoid leading-zero days
- Community-event `Why It Stands Out` language now translates internal factors into reader-facing explanations, and butterfly/trail/conservation listings get a specific outdoor/conservation explanation rather than raw factor labels like `Policy matter`
- Narrowed the community-event scoring rule from broad `policy` to `public policy` / `policy forum` so unrelated words such as `appropriate footwear` do not create policy signals
- Deployed the public-cleanup and writing pass to Freehostia, mirrored changed public files under both the flat root and `web/` paths, and refreshed production through runs `#211`-`#213`; live scans of `/`, `/calendar`, `/archive`, `/topics`, `/opinion`, and `/events/5509/flutterby-butterfly-walk` found no public score/rank/scoring/coverage-tier language
...
- internally prioritized community events now have first-party local pages at `/events/{id}/{slug}` instead of only linking out to the Wareham source page
...
- the homepage now includes an `Around Town` section fed by internally prioritized community events
...
- community-event pages now render as short local briefs with a stronger summary, a `What to Know` section, a reader-facing `Why It Stands Out` note, related links, and linked topic chips
...
- Keep scanning public pages after scoring/desk changes so internal score, rank, factor, override, worker, and desk language stays confined to protected/admin surfaces.
